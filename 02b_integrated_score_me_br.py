...
# MAGIC
# MAGIC Consome `teste.integrated_score_chile` (produzida pelo NB02b do pipeline Chile)
...
# MAGIC ### Dependencias
# MAGIC
# MAGIC - `teste.integrated_score_chile` (NB02b Chile) — scores e limites MI+ME
# MAGIC - `teste.apply_model_me_br` (NB02 ME BR) — tabela destino de ambos os MERGEs
# MAGIC - `teste.customer_top_category_me_br` (NB01b) — categoria principal por cliente/trimestre
# MAGIC - `teste.category_limit_me_br` (NB01b) — teto de limite por categoria/trimestre
# MAGIC - `financeiro.dw_tab_parametro_cotacao_cambial` — taxa CLP/USD para conversao do teto
...
# MAGIC %sql
# MAGIC SELECT
# MAGIC   (SELECT COUNT(*) FROM teste.integrated_score_chile)                     AS isc_total_linhas,
# MAGIC   (SELECT CAST(MAX(reference_month) AS STRING)
# MAGIC    FROM teste.integrated_score_chile)                                     AS isc_safra_max,
# MAGIC   (SELECT COUNT(*) FROM teste.apply_model_me_br)                          AS me_total_linhas,
# MAGIC   (SELECT CAST(MAX(reference_month) AS STRING)
# MAGIC    FROM teste.apply_model_me_br)                                          AS me_safra_max,
# MAGIC   -- Clientes em comum esperados no Passo 1
# MAGIC   (SELECT COUNT(DISTINCT isc.id_customer_me)
# MAGIC    FROM teste.integrated_score_chile isc
# MAGIC    INNER JOIN teste.apply_model_me_br me
...
# MAGIC   (SELECT COUNT(*) FROM teste.customer_top_category_me_br)                AS ctc_total_linhas,
# MAGIC   (SELECT CAST(MAX(reference_quarter) AS STRING)
# MAGIC    FROM teste.customer_top_category_me_br)                                AS ctc_trimestre_max,
# MAGIC   (SELECT COUNT(*) FROM teste.category_limit_me_br)                       AS cl_total_linhas,
# MAGIC   (SELECT CAST(MAX(reference_quarter) AS STRING)
# MAGIC    FROM teste.category_limit_me_br)                                       AS cl_trimestre_max
...
# MAGIC -- ====================================================================
# MAGIC MERGE INTO teste.apply_model_me_br AS target
# MAGIC USING (
# MAGIC   SELECT
...
# MAGIC     limit_mi_usd,
# MAGIC     limit_mi_clp
# MAGIC   FROM teste.integrated_score_chile
# MAGIC ) AS source
...
# MAGIC -- Mesma logica de forward-fill do NB02 (financeiro.dw_tab_parametro_cotacao_cambial)
# MAGIC -- ====================================================================
...
# MAGIC   FROM teste.customer_top_category_me_br ctc
# MAGIC   LEFT JOIN teste.category_limit_me_br cl
...
# MAGIC   FROM teste.apply_model_me_br am
...
# MAGIC SELECT
# MAGIC   -- Volume
# MAGIC   (SELECT COUNT(*) FROM teste.apply_model_me_br)                          AS total_linhas,
# MAGIC   (SELECT COUNT(DISTINCT reference_month) FROM teste.apply_model_me_br)   AS total_safras,
# MAGIC   (SELECT CAST(MAX(reference_month) AS STRING)
# MAGIC    FROM teste.apply_model_me_br)                                          AS safra_max,
# MAGIC
# MAGIC   -- PASSO 1: clientes MI+ME vs apenas-ME
# MAGIC   (SELECT COUNT(*) FROM teste.apply_model_me_br
...
# MAGIC      AND reference_month = (SELECT MAX(reference_month) FROM teste.apply_model_me_br))
...
# MAGIC   (SELECT ROUND(AVG(credit_limit), 2) FROM teste.apply_model_me_br
# MAGIC    WHERE reference_month = (SELECT MAX(reference_month) FROM teste.apply_model_me_br))
# MAGIC                                                                           AS avg_credit_limit,
# MAGIC   (SELECT ROUND(AVG(credit_limit_end), 2) FROM teste.apply_model_me_br
...
# MAGIC   (SELECT COUNT(*) FROM teste.integrated_score_chile
# MAGIC    WHERE reference_month = (SELECT MAX(reference_month) FROM teste.integrated_score_chile))
...
# MAGIC %sql
# MAGIC select * from teste.apply_model_me_br