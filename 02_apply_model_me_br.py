...
# MAGIC ### Dependencias
# MAGIC
# MAGIC - `teste.abt_inference_me_br` (NB1) — features e scores individuais
# MAGIC - `teste.portfolio_abt_group_me_br` (NB1) — portfolio_score
# MAGIC - `dados_mestres.dbpessoa_tab_pessoa` — RUT (num_registro_comercial)
# MAGIC - `financeiro.dw_tab_parametro_cotacao_cambial` — taxa CLP/USD
# MAGIC - `teste.customer_top_category_me_br` (NB1b) — categoria principal por cliente/mes
# MAGIC - `teste.category_limit_me_br` (NB1b) — teto de limite por categoria/mes
...
# MAGIC %sql
# MAGIC
# MAGIC CREATE TABLE IF NOT EXISTS teste.apply_model_me_br (
...
# MAGIC     ) AS rn
# MAGIC   FROM dados_mestres.dbpessoa_tab_pessoa tp
# MAGIC   LEFT JOIN (
...
# MAGIC   FROM teste.abt_inference_me_br AS inf
...
# MAGIC    WHERE updated_at = (SELECT MAX(updated_at) FROM teste.apply_model_me_br
# MAGIC                        WHERE updated_at IS NOT NULL))                      AS linhas_ultima_execucao,
# MAGIC   (SELECT MIN(reference_month) FROM teste.apply_model_me_br)              AS safra_min,
# MAGIC   (SELECT MAX(reference_month) FROM teste.apply_model_me_br)              AS safra_max,
# MAGIC   (SELECT COUNT(DISTINCT reference_month) FROM teste.apply_model_me_br)   AS total_safras,
# MAGIC   (SELECT COUNT(DISTINCT id_customer) FROM teste.apply_model_me_br)       AS total_clientes,
# MAGIC   (SELECT COUNT(*) FROM (
# MAGIC     SELECT reference_month, id_customer, COUNT(*) AS cnt
# MAGIC     FROM teste.apply_model_me_br
# MAGIC     GROUP BY reference_month, id_customer HAVING cnt > 1
# MAGIC   ))                                                                       AS duplicatas,
# MAGIC   (SELECT MAX(reference_month) FROM teste.abt_inference_me_br)            AS abt_max_safra,
# MAGIC   (SELECT add_months(MAX(reference_month), 1)
# MAGIC    FROM teste.abt_inference_me_br)                                         AS expected_apply_max,
# MAGIC   (SELECT COUNT(*) FROM teste.apply_model_me_br
# MAGIC    WHERE adjusted_score IS NULL
# MAGIC      AND updated_at = (SELECT MAX(updated_at) FROM teste.apply_model_me_br
...
# MAGIC      AND reference_month = (SELECT MAX(reference_month) FROM teste.apply_model_me_br)) AS band_baixo,
# MAGIC   (SELECT COUNT(*) FROM teste.apply_model_me_br
# MAGIC    WHERE score_band = '2-MEDIO'
# MAGIC      AND reference_month = (SELECT MAX(reference_month) FROM teste.apply_model_me_br)) AS band_medio,
# MAGIC   (SELECT COUNT(*) FROM teste.apply_model_me_br
# MAGIC    WHERE score_band = '3-ALTO'
# MAGIC      AND reference_month = (SELECT MAX(reference_month) FROM teste.apply_model_me_br)) AS band_alto,
